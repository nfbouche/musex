...
def test_catalog_name(settings_file):
    ...
    with pytest.warns(
        UserWarning, match="catalog name should contain only ascii letters "
    ):
...
def test_catalog(mx):
    ...
    res = phot.select(
        wcs=mx.muse_dataset.white.wcs, columns=[phot.idname], margin=2 / 0.2
    )
    assert len(res) == 8

    # Test selection with a WCS and a mask
    res = phot.select(
        wcs=mx.muse_dataset.white.wcs,
        mask=np.ones(mx.muse_dataset.white.data.shape, dtype=bool),
    )
    assert len(res) == 0
...
def test_update_column(mx, caplog):
    ...
    mycat.insert(t)
    assert caplog.records[0].message == "The column with-dash was renamed to with_dash."
...
def test_update_rows(mx):
    ...
    assert len(mycat) == 4
    assert_allclose(
        [o["ra"] for o in mycat.select(columns=["ra"])], np.arange(4, dtype=float)
    )
...
def test_export_marz(mx):
    ...
    with fits.open(f"{outdir}/marz-my_cat-hdfs.fits") as hdul:
        ...
        assert_array_equal(
            hdul["DETAILS"].data["REFSPEC"], ["MUSE_PSF_SKYSUB", "MUSE_TOT_SKYSUB"]
        )
...
def test_export_marz_from_sources(mx):
    ...
    with fits.open(f"{outdir}/marz-my_oricat-hdfs.fits") as hdul:
        ...
        assert_array_equal(
            hdul["DETAILS"].data["REFSPEC"], ["ORI_CORR_3_SKYSUB", "ORI_CORR_4_SKYSUB"]
        )
...
def test_export_sources_origin(mx):
    ...
    assert list(src.spectra.keys()) == [
        "ORIG_MUSE_SKY",
        "ORIG_MUSE_TOT_SKYSUB",
        "ORIG_MUSE_WHITE_SKYSUB",
        "ORIG_MUSE_TOT",
        "ORIG_ORI_CORR",
        "ORIG_MUSE_PSF_SKYSUB",
        "ORIG_MUSE_PSF",
        "ORIG_ORI_SPEC_3",
        "ORIG_ORI_CORR_3_SKYSUB",
        "ORIG_ORI_CORR_3",
        "MUSE_TOT",
        "MUSE_WHITE",
        "MUSE_PSF",
...
def test_export_match(mx):
    ...
    tbl = phot.select(
        phot.c.id == mycat.c.photutils_id, columns=["id", "ra", "dec", "area"]
    ).as_table()
    ...
    tbl = orig.select(
        orig.c.ID == mycat.c.origin_id, columns=["ID", "ra", "dec", "purity"]
    ).as_table()
    ...
    # add mask dataset
    maskds = [
        "origin" if row["origin_id"] else "photutils_masks" for row in mycat.select()
    ]
